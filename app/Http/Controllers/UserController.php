...
class UserController extends Controller
{
	...
	public function login(ValidaLogin $request)
	{
		...
		if (Auth::attempt(['email' => $request->email, 'password' => $request->senha], $request->remember)) {
	...
	public function Registro(ValidaRegistro $request)
	{
		...
		if ($registrar) :
			...
			return redirect('/login')->with('sucesso', 'Conta Criada! Aguarde até que nós a ativemos!!');

		else :

			return redirect('/cadastro')->with('erro', 'Conta Não Criada! Houve um erro contate o administrador!');
