...
');
    ...
    public function editaDespesaIndex(Request $request){
        ...
        $id = request('id');
        $edit = DB::select('select * from despesas where id = :id and id_usuario = :usuario', 
            [
                ':id' => $id,
...
');
    ...
    public function verDespesas(){
        ...
        $despesas = DB::table('despesas')
                        ...
                        ->select('despesas.descricao', 'despesas.valor','despesas.data',
                                 'despesas.anexo', 'despesas.id')->orderBy('despesas.id', 'desc')->Paginate(5);
        ...
        return view('SistemaDespesas.Anterior', compact('valordespesa'));
         ...
         function AjaxDespesas(Request $request)
            ...
            $despesas = DB::table('despesas')
                        ->join('usuarios', 'despesas.id_usuario', '=', 'usuarios.id')
                        ->select('despesas.descricao', 'despesas.valor','despesas.data',
                                 'despesas.anexo', 'despesas.id')->orderBy('despesas.data', 'desc')->Paginate(5);
